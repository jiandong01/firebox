...
from firebox.api import models
from firebox.constants import TIMEOUT, ENVD_PORT, FILE_ROUTE, DOMAIN
from firebox.sandbox.code_snippet import CodeSnippetManager, OpenPort
from firebox.sandbox.env_vars import EnvVars
from firebox.sandbox.filesystem import FilesystemManager
from firebox.sandbox.process import ProcessManager, ProcessMessage
from firebox.sandbox.sandbox_connection import SandboxConnection
from firebox.sandbox.terminal import TerminalManager
...
class Sandbox(SandboxConnection):
    """
    Firebox cloud sandbox gives your agent a full cloud development environment that's sandboxed.
    ...
    Check usage docs - https://firebox.dev/docs/sandbox/overview
    ...
    def __init__(
        ...
        :param api_key: The API key to use, if not provided, the `FIREBOX_API_KEY` environment variable is used
    ...
    def reconnect(
        ...
        :return: Reconnected Sandbox instance
        """
    ...
    def upload_file(self, file: IO, timeout: Optional[float] = TIMEOUT) -> str:
        ...
        :return: The path of the uploaded file in the sandbox
        """
        files = {"file": file}
